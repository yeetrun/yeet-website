---
title: Troubleshooting
description: Common failures and how to fix them.
---

Start with the symptom you see. If this is a fresh host, run
[First-Run Validation](/docs/getting-started/first-run-validation) after fixing
the first connection problem.

## yeet cannot reach catch

Confirm you are targeting the catch hostname, not the SSH machine hostname:

```bash
CATCH_HOST=<catch-host> yeet status
yeet --host=<catch-host> status
```

Then check:

- The catch tsnet node exists in Tailscale.
- The node has a tag such as `tag:catch`.
- Your device or admin tag can reach the catch tag on TCP port `41548`.
- The catch service is running on the host.

If you have the Tailscale CLI installed locally, this can help confirm the
tagged node is visible:

```bash
yeet list-hosts
```

See [Tailscale Setup](/docs/concepts/tailscale#first-time-host-setup) for the
tailnet policy shape.

## `yeet init` says catch must be tagged

Catch rejects user-owned tsnet nodes. It must join the tailnet with a tag-based
identity.

Fix the Tailscale policy first, then rerun init:

```bash
yeet init root@<machine-host>
```

For repeatable setup, pass an OAuth client secret that can assign the catch
tag:

```bash
yeet init --install-docker --ts-client-secret=<secret> root@<machine-host>
```

Advanced users can pass a preauthorized Tailscale auth key instead:

```bash
yeet init --install-docker --ts-auth-key=<key> root@<machine-host>
```

## `yeet init` got a different catch hostname

Tailscale assigned a suffix such as `catch-1` because another device already
owns the requested hostname.

Use the assigned catch host:

```bash
yeet --host=catch-1 status
```

Or remove the stale/conflicting Tailscale device and rerun `yeet init`.

## Docker payloads fail

Container payloads require Docker on the catch host. On Debian/Ubuntu-style
hosts, install it during bootstrap:

```bash
yeet init --install-docker root@<machine-host>
```

If local image pushes do not show up on the host, confirm Docker's containerd
snapshotter is enabled. `yeet init` configures this during catch install.

For Compose services:

```bash
yeet info <svc>
yeet logs <svc>
yeet ssh -- docker ps
```

See [Containers](/docs/payloads/containers) for update and image workflows.

## Published port changes are rejected

`yeet service set` protects the full published-port list. Include every mapping
you want to keep:

```bash
yeet service set web -p 80:80 -p 443:443
```

To replace the list or clear it, use `--publish-reset`:

```bash
yeet service set web --publish-reset -p 443:443
yeet service set web --publish-reset
```

Use `yeet info web` before changing ports.

## Service networking does not work

First confirm the service has the network mode you expect:

```bash
yeet info <svc>
yeet ip <svc>
```

For `svc` networking, services should receive private service-network
addresses. For `lan`, the host network must support macvlan and DHCP. A
WAN-only VPS usually should not use `lan`.

If the service was running before a network change, restart it:

```bash
yeet restart <svc>
```

See [Networking](/docs/concepts/networking) before changing modes on a shared
host.

## Yeet DNS names do not resolve

Yeet DNS only publishes names for services and VMs using `svc` networking.

Check the resolver from a service or VM on the service network:

```bash
dig @192.168.100.1 <svc> A +short
dig @192.168.100.1 <svc>.yeet.internal A +short
```

If the query returns nothing, confirm `yeet info <svc>` shows a service-network
IP. See [DNS](/docs/concepts/dns) for records and resolver behavior.

## VM payloads do not start

VMs require a KVM-capable Linux catch host. Check the host:

```bash
yeet ssh -- sh -lc '
test -e /dev/kvm && echo kvm=yes || echo kvm=no
test -e /dev/net/tun && echo tun=yes || echo tun=no
for c in qemu-img zstd e2fsck resize2fs mount umount ip; do
  command -v "$c" >/dev/null && echo "$c=yes" || echo "$c=no"
done
'
```

If KVM is missing, use containers, binaries, scripts, and cron jobs on that
host. If KVM and TUN/TAP exist but tools are missing on Debian/Ubuntu, rerun
init with VM tools:

```bash
yeet init --install-vm-tools root@<machine-host>
```

See [VMs](/docs/payloads/vms) for image cache and console diagnostics.

## Binary architecture mismatch

Build binaries for the catch host architecture:

```bash
GOOS=linux GOARCH=amd64 go build -o ./bin/<svc> ./cmd/<svc>
yeet run <svc> ./bin/<svc>
```

Use the host's actual architecture when it is not amd64.

## `edit` fails without a TTY

`yeet edit` and `yeet env edit` launch an editor. Run them from an interactive
shell.

## Staged changes are confusing

Inspect the staged payload:

```bash
yeet stage <svc> show
```

Apply it:

```bash
yeet stage <svc> commit
```

Or discard it:

```bash
yeet stage <svc> clear
```

## `copy` destination errors

One side of a copy must be remote, such as `<svc>:path` or `<vm>:path`.

Examples:

```bash
yeet copy ./config.yml <svc>:config/config.yml
yeet copy ./configs/ <svc>:config/
yeet copy <vm>:"/var/log/*.log" ./logs/
```

Remote service paths are relative to the service data directory.

## Service type mismatch

A service type is fixed after the first install. If you installed a service as
Compose, you cannot later replace it with a binary using the same service name.

Choose a new service name, or remove the old service first:

```bash
yeet rm <svc>
```

Use `--clean-data` only when you also want to delete the service data.
