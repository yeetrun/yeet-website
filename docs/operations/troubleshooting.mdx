...
- If a netns-backed compose service becomes unreachable after networking
  changes, catch will now reconcile that service at startup by restarting the
  compose project when it detects the containers are still attached to an old
  service netns. Check the catch logs for a message saying it restarted the
