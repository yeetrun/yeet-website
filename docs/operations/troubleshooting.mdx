...
- `svc` or `lan` combined with `iso`, or any second Yeet-managed mode on an ISO VM.
- Native binary, script, or cron payloads.
...
For a non-VM service that needs a tailnet identity, use `iso,ts`. For a VM,
