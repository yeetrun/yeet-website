...
compose images, add `--pull` (or use `yeet docker update <svc>`) when you want to
refresh one service. Use `yeet docker update --outdated` to batch-refresh only
compose services reported by `yeet docker outdated`.
