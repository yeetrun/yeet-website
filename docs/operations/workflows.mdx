...
hosts with the required VM devices available, missing VM packages can be
installed with `apt-get` when the install is interactive. ZFS is optional; use
`--zfs` only when you want ZFS-backed VM disks.
