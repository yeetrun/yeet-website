...
the serial console for boot diagnostics. `yeet vm images update` refreshes all
official image families and removes old unreferenced image versions when it is
safe; pass a specific `vm://` payload to refresh one family. Use `yeet vm
images prune --dry-run` to preview manual cleanup. Remove a VM disk with:
