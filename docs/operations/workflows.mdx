...
For the full container guide, see [Containers](/docs/payloads/containers).
...
Use `--net=lan` when the service should request a LAN address through macvlan
and your host network supports it:
...
For the full VM guide, see [VMs](/docs/payloads/vms).

Use a `vm://` payload when you want a long-lived Linux guest managed by yeet:
...
Official catalog examples include Ubuntu and NixOS:
...
hosts with the required VM devices available, pass `--install-vm-tools` to
install missing VM packages with `apt-get`. ZFS is optional; use `--zfs` only
when you want ZFS-backed VM disks.

Use `lan` when the VM should get a LAN address through DHCP:

```bash
yeet run <vm> vm://ubuntu/26.04 --net=lan
```

Use `svc,lan` when the VM should be on the catch host's LAN and still have a
private yeet management path for SSH. In this mode, the VM's normal outbound
traffic uses the LAN route while yeet management traffic stays available on the
service network:
...
the serial console for boot diagnostics; detach by pressing Enter, then typing
`~.`. The official VM catalog is loaded from `yeet-vm-images` at runtime, and
`yeet vm images catalog` lists the VM payloads the selected catch host can run,
even before any image is cached.
`yeet vm images update` refreshes all official image families and removes old
unreferenced image versions when it is safe; pass a specific `vm://` payload to
refresh one family. Use `yeet vm images prune --dry-run` to preview manual
cleanup. Remove a VM disk with:

```bash
yeet rm --clean-data <vm>
```

Change VM resources after creation by stopping the VM first:

```bash
yeet stop <vm>
yeet vm set <vm> --cpus=6 --memory=6g --disk=128g
yeet vm set <vm> --net=lan
yeet start <vm>
```

Disk changes are grow-only. ZFS-backed VMs grow the VM ZVOL and then the guest
ext4 filesystem.
...
See [ZFS](/docs/concepts/zfs) for dataset, snapshot, and VM clone behavior.
...
The file passed with `--config` must already exist and contain the service
entries you want to update.
...
For the complete ZFS model, see [ZFS](/docs/concepts/zfs).

## Clone-first recovery

When you are unsure what a recovery point contains, clone it into a new service
first and inspect the clone:
...
In-place restore creates a pre-restore recovery point before changing ZFS
state. Use `--start` if the original service should start again after restore.
...
For host service details, see [Binaries & Scripts](/docs/payloads/binaries).
...
For timer behavior and update flow, see [Cron Jobs](/docs/payloads/cron-jobs).
