...
Debian/Ubuntu hosts with the required VM devices available, pass
`--install-vm-tools` to install missing VM packages with `apt-get`. ZFS is
optional; use `--zfs` only when you want ZFS-backed VM disks.
...
the serial console for boot diagnostics. `yeet vm images catalog` lists the
VM payloads the selected catch host can run, even before any image is cached.
