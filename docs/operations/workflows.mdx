...
the serial console for boot diagnostics. `yeet vm images update` removes old
unreferenced image versions when it is safe; use `yeet vm images prune
--dry-run` to preview manual cleanup. Remove a VM disk with:
