...
The path is absolute on the catch host. The parent directory must already exist;
yeet can create the final service directory. Each service root contains `bin`,
`run`, `env`, and `data`.
...
must pass either `--copy` or `--empty`.
