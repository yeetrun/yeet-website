---
title: Networking
description: Choose service reachability, DNS, and outbound internet paths.
---
...
egress through the catch host while adding a tailnet identity.
...
private service networking, LAN presence, or a Tailscale identity.
...
`lan` attaches a service or VM to the catch host's LAN with macvlan. The
workload requests addressing through DHCP.
...
interface make sense. A WAN-only VPS usually should not use `lan`.
...
workstation on its guest LAN IP for `yeet ssh <vm>` to connect directly.
...
If the service has no outbound internet:
