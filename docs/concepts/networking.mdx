...
- If a named service netns is recreated, catch now detects stale compose
  containers and restarts the whole compose project so it rejoins the current
  netns.
- catch also performs this reconciliation pass at startup for netns-backed
  docker services and logs when it restarts a service for this reason.
