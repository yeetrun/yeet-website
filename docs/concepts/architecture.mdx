...
- **catch**: daemon that installs, runs, and monitors services and VMs.
- **catchrpc**: JSON-RPC + WebSocket layer for exec/events.
- **internal registry**: Docker registry served by catch for image pushes.
- **VM image cache**: host-local cache for yeet-owned Firecracker VM bundles.
...
4) Catch installs or updates the service (systemd, docker compose, or VM).
...
[Networking](/docs/concepts/networking) for details and
[Tailscale](/docs/concepts/tailscale) for how RPC and service netns are wired.

## Authorization

RPC access is gated by Tailscale identity:

- Catch must run as a tagged Tailscale node, such as `tag:catch`.
- Tailnet ACLs or grants decide which users, devices, and tagged nodes can
  reach catch RPC.
- Untagged, user-owned catch nodes are rejected; catch is server
  infrastructure, not an end-user device.

See [Tailscale](/docs/concepts/tailscale) for setup and tailnet behaviors.
