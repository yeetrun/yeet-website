---
title: Service Types
description: Payload detection and installation behavior for containers, VMs, and host services.
---

Yeet detects payload type and installs the service accordingly. A service can
be a containerized app, a host systemd service, a cron-style job, or a
Firecracker-backed VM guest.
...
- `yeet init` checks the host for KVM, TUN/TAP, and required VM tooling; ZFS is
  optional unless you create VMs with `--zfs`.
...
image bundle. Guest apt upgrades do not manage the boot kernel, bootloader,
initrd, or init shim; publish a new VM image bundle for those updates.
