...
During `yeet init`, catch starts its own embedded tsnet node for RPC. If that
node needs first login, `yeet init` prints the Tailscale login URL. For
unattended host bootstrap, pass an auth key for the catch node with
...
If your tailnet ACLs grant server access through tags, make sure unattended
auth keys assign those tags. A catch node can authenticate successfully but
still be unreachable from your laptop if it joins as a user-owned device and
your ACLs only permit tagged server devices.
