...
Yeet DNS is scoped to yeet-managed hosts and the service network. It is not
meant to replace your LAN DNS or public DNS; it acts as the single resolver
that knows yeet-local names and forwards everything else.
...
`127.0.0.11`. On hosts configured by `yeet init --install-docker`, Docker's
daemon defaults send that embedded resolver to catch DNS and add
`yeet.internal` as a search domain. Compose files do not need to set `dns:` or
`dns_search:` for yeet DNS. If a compose file explicitly sets DNS, that service
owns its resolver configuration and may bypass yeet DNS.
