...
The first VM created on a pool for an image version prepares a shared ZFS image
base. Later VMs on the same pool and image version clone that base instead of
writing the root filesystem again.
