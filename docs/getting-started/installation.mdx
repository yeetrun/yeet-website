...
layout and upgrades catch without asking the storage questions again. The
default is stability, not "surprise, your data moved."
...
bridge support too. `--install-docker` and `--install-vm-tools` are unattended
yes flags for the package prompts.

If you use a non-root SSH user, yeet runs the remote install with sudo.

## Pre-answer package prompts

For interactive setup, run plain `yeet init`. If Docker or VM tools are missing
on supported hosts, init asks before installing them.

For unattended container bootstrap on a Debian/Ubuntu-style host, pre-answer the
Docker prompt:
...
For unattended VM payload setup on a KVM-capable host, pre-answer Docker and VM
tool prompts:
...
providers do not expose nested virtualization. That is a host constraint, not a
motivational challenge. On hosts that are not VM capable, yeet warns and
containers, binaries, scripts, and cron jobs still work.

## Run unattended setup

For repeatable catch enrollment, pass the Tailscale OAuth client secret:
...
If your automation also needs host packages installed, add the package flags it
...
after the node has the required tag. If the tag is wrong, the daemon may exist,
but the access model is still not valid.
