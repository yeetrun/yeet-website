...
VM payloads require KVM and TUN/TAP on the catch host. Many VPS providers do
not expose nested virtualization; that is a host limitation, not a failed yeet
install.

```bash
yeet ssh -- sh -lc 'test -e /dev/kvm && echo kvm=yes || echo kvm=no; test -e /dev/net/tun && echo tun=yes || echo tun=no'
```

If both are available, create a disposable VM:
