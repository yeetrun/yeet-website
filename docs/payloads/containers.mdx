...
internal registry, then installed on the catch host.
...
the private service network and `ts` for Tailscale service netns behavior.
...
including how short names and `*.yeet.internal` names resolve inside service
network namespaces.
...
under the managed service root.
...
Debian/Ubuntu hosts and configures the snapshotter during catch install. It
also configures Docker daemon DNS defaults so containers without explicit
compose `dns:` settings use catch DNS for `svc` service discovery, public DNS,
and tailnet names. See [DNS](/docs/concepts/dns) for resolver behavior,
