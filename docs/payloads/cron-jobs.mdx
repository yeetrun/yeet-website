...
Cron jobs cannot use `iso` networking while timer payloads run as root-owned
host services. Host root can leave or reconfigure a network namespace, so Yeet
rejects that unsafe combination. Put an untrusted scheduled workload in a
container or VM, or wait for future non-root service sandboxing.
