---
title: Cron Jobs
description: Install scheduled jobs as systemd timers on the catch host.
---
...
write persistent state or process files under their managed data directory.
...
`--clean-data` removes the managed service data root in addition to the
scheduled job. Read the confirmation prompt before accepting.
