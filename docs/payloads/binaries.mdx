...
Native binaries and scripts cannot use `--net=iso`. The native ISO policy path
is not implemented, so yeet rejects the combination instead of presenting an
incomplete isolation boundary. Use a container-backed payload or VM for ISO.
