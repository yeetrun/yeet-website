...
The official image includes built-in TUN, nftables, conntrack, and IPv4 NAT
support, plus `iptables` and `nftables` userspace tools, so guest-installed
router and VPN services can manage their own firewall and routing state.
Tailscale is not preinstalled.
