...
the configured maximum. This does not increase available host memory.
...
Stop the VM before changing CPU, memory, disk size, networking, or host VMM
isolation:
...
## Isolate the host VMM

New VMs launch Firecracker through the matching Firecracker jailer. The jailer
builds a private filesystem view, provides only the VM resources needed for the
boot, and drops Firecracker to the static `yeet-vm` host account. This account
is separate from users inside the guest.

Existing VMs keep their current launch mode during an upgrade. Check one VM:

```bash
yeet info <vm>
```

The `VMM isolation` row shows `jailer` or `legacy-root`. Migrate a stopped VM
explicitly:
...
Migration keeps the VM service root, disk backend, snapshots, network mode,
and guest users. It also delegates the VM's TAP devices, writable disk, and
Firecracker checkpoint files to the host VMM account. Custom service roots and
ZFS-backed VM roots use the same workflow.

If the VM does not start after migration, stop it and restore the previous host
launch mode:

```bash
yeet stop <vm>
yeet vm set <vm> --vmm-isolation=legacy-root
yeet start <vm>
```

Jailer migration requires the VM's cached image bundle to contain a jailer that
matches its Firecracker version. Catch fails before changing the VM when that
artifact is missing; `yeet vm images update` fetches a newer bundle when the VM
image catalog provides one.
...
Catch creates the `yeet-vm` system account when the first jailed VM is prepared.
No login shell or home directory is created for the account.
...
Re-importing the same image name affects new VMs only. Existing VMs keep the
image version from their original create. That is the point: a running guest
should not mutate because an image ref moved.
