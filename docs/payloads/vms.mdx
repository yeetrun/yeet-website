...
uses that network's gateway and DNS for ordinary internet access:
...
back and rerun the same `yeet run` command.
...
identity, install and configure Tailscale inside the guest.
