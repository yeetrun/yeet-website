...
Yeet uses Firecracker jailer mode by default. The jailer builds a private
filesystem view, provides only the VM resources needed for boot, and runs
Firecracker as the static `yeet-vm` host account. This account is separate from
users inside the guest.

Each VM stores its host VMM isolation mode. Catch upgrades do not change it.
Check the mode with:
...
The `VMM isolation` row shows `jailer` or `legacy-root`. Set jailer mode while
the VM is stopped:
...
Changing the mode keeps the VM service root, disk backend, snapshots, network
mode, and guest users. Jailer mode delegates the VM's TAP devices, writable
disk, and Firecracker checkpoint files to the host VMM account. Custom service
roots and ZFS-backed VM roots use the same workflow.

`legacy-root` runs Firecracker as root for compatibility. Set it while the VM is
stopped:
...
Jailer mode requires the VM's cached image bundle to contain a jailer that
matches its Firecracker version. Catch fails before changing the mode when that
artifact is missing. `yeet vm images update` downloads the current catalog
bundle.
