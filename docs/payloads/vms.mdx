...
runtime-state format. The upgrade error lists each affected VM and path. Archive
or remove its `memory.bin` and `firecracker-state.bin` files, then remove the old
recovery point:
...
To keep the ZFS disk snapshot after archiving or removing the runtime-state
files, run this as root on the catch host instead:

```bash
zfs set com.yeetrun:checkpoint=disk <pool/dataset>@<snapshot>
```

Retry the catch upgrade after every path in the upgrade error is handled.
