...
creating a Tailscale OAuth client that can assign the service tags you plan to
use with `--net=ts`, then stores the `tskey-client-...` secret on the target
host. Use `--client-secret` to skip the prompt.
...
while the remote install runs. The catch node must end up tagged, such as
`tag:catch`; user-owned catch nodes are rejected. For unattended setup, pass a
preauthorized Tailscale auth key for the catch node with
`--ts-auth-key=<key>`.
