...
Stop the VM before changing VM resources, networking, or host VMM isolation:
...
Migrate an existing VM to the unprivileged Firecracker jailer, or roll it back
to the legacy root launch mode:
...
`yeet info <vm>` reports the active mode in the `VMM isolation` row.
