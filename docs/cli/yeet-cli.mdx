...
directory, such as `svc:dir/`, `devbox:.ssh/`, or an existing local directory.
...
`devbox:".ssh/id*"` are VM-only. By default, `yeet copy` behaves like
...
- For VM services with `svc` or `svc,lan`, yeet proxies SSH through the catch
  host to the VM's private management IP. For LAN-only VMs, yeet connects
  directly to the guest LAN IP. It prints the selected path to stderr before
  starting SSH.
- Use `--force-proxy` to force the catch-host proxy path for a VM.
