...
`default_user`, `guest_system_init`, and `metadata_driver`. Custom image rootfs
contents should include the guest tools you expect to use, such as SSH, `rsync`
for `yeet copy`, and terminal definitions for operator `TERM` values such as
`xterm-ghostty`. Remove an imported ref with `yeet vm images rm <name> --yes`.
