...
config, network, runtime, images) and the Network section includes published
ports when catch reports them. Use `--format=json` or `--format=json-pretty`
for machine-readable output with structured port mappings.
