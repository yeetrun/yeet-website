...
For normal services, the service root must be ZFS-backed. For VMs, yeet
snapshots the VM disk; `--full` also writes Firecracker state and memory
checkpoint files for advanced recovery workflows. This command creates recovery
points, but restore and clone commands are not implemented yet.
