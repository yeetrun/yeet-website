...
On hosts without Docker, interactive `init` prompts to install Docker. For
scripted setup, pass `--install-docker`. During catch install, yeet also
configures Docker's containerd snapshotter so pushed images are visible to the
host Docker daemon.
