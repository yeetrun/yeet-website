...
- For VM services, yeet proxies the SSH connection through the catch host to
  reach the guest address reported by catch. This keeps `yeet ssh <vm>` useful
  even when the VM is on the catch host's LAN rather than your workstation's
  LAN.
