...
For recipes, start with [Workflows](/docs/operations/workflows). This page is
for lookup after you know the shape of the job. Reference docs are not a
personality test; nobody wins by reading them top to bottom first.
...
That distinction matters most when you are already stressed.
...
They are for storage state, not for wishing every risky change had been
documented.
...
rerun `yeet init`. Re-running init is an upgrade path, not a storage migration
surprise.
...
Automation should declare whether services move; a hidden default here would be
a bad joke.
...
network reachability to catch's tsnet address.
