...
which restores a ZFS service root or VM disk. `--mode=full` is VM-only; this
release can capture and inspect full VM checkpoints but refuses full restore
before disk mutation. Use `--mode=disk` for VM disk recovery.
