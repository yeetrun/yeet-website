...
host. Rootfs-only imports use yeet's managed kernel and Firecracker; add
`--allow-local-kernel` only when the bundle includes `vmlinux`. Remove an
imported ref with `yeet vm images rm <name> --yes`.
