...
Show or refresh the VM image cache on the selected catch host:
...
`vm://ubuntu/26.04` plus any imported local image refs. `yeet vm images update`
downloads and verifies the latest Ubuntu image bundle used for future VM
creates. A missing image is downloaded automatically on the first VM create.
Existing VMs are not rebuilt or modified.
