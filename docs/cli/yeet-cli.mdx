...
Create a manual recovery point for a ZFS-backed VM. This is a VM-focused
shortcut for `yeet snapshots create <vm>`:
...
Disk snapshots are the default. If the VM is running, catch pauses the
Firecracker VM, snapshots the ZVOL, then resumes it. `--full` also stores
Firecracker state and memory checkpoint files. Manual VM snapshots require
snapshots to be enabled for the VM service. `--full` requires the VM to be
running.
...
`yeet ssh`. By default, `yeet copy` behaves like `rsync -avz` (archive +
verbose + compress). Alias: `cp`.
...
selected catch host before any image is cached. It includes official image
families such as `vm://ubuntu/26.04` and `vm://nixos/26.05`, plus imported
local image refs.
...
and verifies the latest official image bundles used for future VM creates, then
removes old unreferenced cache entries when it is safe. Pass a specific payload,
such as `vm://ubuntu/26.04` or `vm://nixos/26.05`, to refresh one image family.
A missing image is downloaded automatically on the first VM create. Existing VMs
are not rebuilt or modified.
