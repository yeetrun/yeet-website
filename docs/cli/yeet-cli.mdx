...
updates. Each update prints a short host/service marker, then streams the same
output as `yeet docker update <svc>`. It does not take a service argument; use
`yeet docker update <svc>` for one service.
