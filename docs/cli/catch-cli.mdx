...
During `catch install`, catch requires Docker and configures Docker's
containerd snapshotter. If Docker is missing, interactive installs prompt to
install it. For non-interactive manual installs on Debian/Ubuntu hosts, set
`CATCH_INSTALL_DOCKER=1` to let catch install Docker before writing the systemd
service.
