...
- `yeet upgrade --force` now stages each catch reinstall with its own temporary
  installer path, avoiding rare collisions when repeated or overlapping
  upgrades target the same host.
- Successful catch reinstalls no longer print a harmless Tailscale proxy
  copy-abort line after the install has completed.

### v0.9.5

- `yeet upgrade --force` no longer prints sample-run or workspace-save guidance
  while it reinstalls catch hosts, so upgrade output stays focused on the
  upgrade plan, host install progress, and real host warnings.
- This release includes the `v0.9.4` upgrade fixes: catch reinstalls avoid
  service-workspace setup prompts and keep each configured catch hostname when
  multiple hosts are upgraded together.

### v0.9.4

- Supersedes `v0.9.3` for installers and upgraders: this keeps the XDG config
  migration, service workspaces, multi-host workspace behavior, and bridge
  reconciliation changes from `v0.9.3`.
- `yeet upgrade --force` no longer opens service workspace setup during catch
  reinstalls, so non-interactive catch upgrades finish cleanly.
- Catch upgrades now reinstall each host with its own configured catch hostname,
  avoiding accidental Tailscale renames such as `-1` suffixes when `CATCH_HOST`
  is set; release binaries are rebuilt with Go 1.26.5.
...
- Multi-host workspaces now behave correctly with `CATCH_HOST`: project-wide
  ...
  the selected host, and existing service bridges are reconciled instead of
  left with stale addresses.
